...
source_hash: "f4369160dda89561577a7577a47804df76f3ec92b717487867257b44de474983"
system: "openclaw"
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work usually means `channels.telegram.apiRoot` was set to the full `/bot
TOKEN
` endpoint. `apiRoot` must be the Bot API root only; `openclaw doctor --fix` removes an accidental trailing `/bot
TOKEN
`.
...
TOKEN
`), `trustedLocalFileRoots` (self-hosted Bot API absolute `file_path` roots)
