...
source_hash: "70feb591a1a02ff7bab7a591ae1200832eede5959fa535cec5718d8459dd5c3c"
system: "openclaw"
...
One secret-hygiene exception: an exact `config set` on a sensitive path (tokens, keys, passwords) never reaches a model. It runs on the deterministic path with a redacted proposal, and the value is masked in the AI-visible history. Prefer `config set-ref <path> env
ENV_VAR
` for secrets.
