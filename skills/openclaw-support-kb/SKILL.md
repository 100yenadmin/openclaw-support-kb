...
3. Use `docs/` pages as factual source, `runbooks/` as process, and `releases/` for version context.
4. Cite source URLs from `docs/` whenever possible. Cite a runbook only for workflow choices.
5. If the question touches config, chain into `openclaw-config-repair`.
6. If local results are missing or stale, run:
   ```bash
   openclaw docs "<query>"
   ```
7. If the user still needs help, chain into `openclaw-support-escalation`.
